...
def setup():
  if vim.eval( '&ft' ) in CLANG_FILETYPES:
    vim.command( 'noremap <silent> <buffer> <s-tab> :py clang_complete.updateParams()<cr>' )
    vim.command( 'snoremap <silent> <buffer> <s-tab> <c-\><c-n>:py clang_complete.updateParams()<cr>' )
